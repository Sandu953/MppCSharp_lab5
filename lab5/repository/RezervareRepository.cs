                ...
                if (connection.State == System.Data.ConnectionState.Open)
                {
                    ...
                    using (SQLiteDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            string nrTelefon = reader.GetString(3);
                            int nrLocuri = reader.GetInt32(4);
                            Rezervare rez = new Rezervare(id, ex, numeClient, nrTelefon, nrLocuri);
                            rezervari.Add(rez);
                        }
                    }
                }
                ...
                if (connection.State == System.Data.ConnectionState.Open)
                {
                    ...
                    using (SQLiteDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            string nrTelefon = reader.GetString(3);
                            int nrLocuri = reader.GetInt32(4);
                            rez = new Rezervare(id, ex, numeClient, nrTelefon, nrLocuri);
            ...
            using (var connection = new SQLiteConnection(dbConnection))
            {
                ...
                if (connection.State == System.Data.ConnectionState.Open)
                {
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        command.Parameters.AddWithValue("@excursie", entity.Excursie.Id);
                ...
                if (connection.State == System.Data.ConnectionState.Open)
                {
                    ...
                    using (SQLiteDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            updateCommand.Parameters.AddWithValue("@ex", entity.Excursie.Id);
